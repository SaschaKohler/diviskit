...
- [Manifest Schema](#design-system-manifest-schema) — `.devin/skills/diviskit-builder/design-system.json` structure
...
**MCP write tools normalize all three pathological forms inside `$variable(...)$` token regions** (`diviskit-mcp/src/wp-client.ts`) — a `diviskit_section_get → diviskit_section_replace` round-trip on legacy broken bytes self-heals. Bytes outside token regions are preserved (so a string-variable value or a code sample documenting the broken form round-trips unchanged). The normalizer runs in two passes: first collapses over-escaped forms to canonical `\"`, then escapes any remaining bare `"` (a negative lookbehind skips already-canonical `\"` so the second pass is idempotent). For non-MCP-touched content (e.g., direct DB writes or custom PHP that bypasses the MCP), recover with a two-target `str_replace` that handles both pathological over-escape forms:
...
**`wp_update_post` hazard (custom PHP / eval-files / non-MCP REST endpoints).** WP's `wp_insert_post` / `wp_update_post` runs `wp_unslash()` on `post_content`, stripping one level of backslashes from ALL `\uXXXX` escapes — not just variable-token quotes: `"` (variable expression `"`), `<` / `>` (HTML `<` / `>` in `content.innerContent`), `&` (HTML `&`), `/` (`/`). All become `u0022` / `u003c` / etc. (no leading backslash) — variable expressions silently fail to resolve and HTML tags render as literal text (e.g. `<p>` becomes the visible string `u003cpu003e`). Damage is irreversible without a pre-`wp_update_post` revision. **When writing variable-token-bearing markup from PHP, bypass `wp_update_post` — write directly via `$wpdb->update($wpdb->posts, ['post_content' => $new], ['ID' => $id], ['%s'], ['%d'])` followed by `clean_post_cache($id)`.** The diviskit-agent plugin uses `wp_slash()` to compensate at REST endpoints; that path is safe. Recovery regex for stripped-backslash damage (run on storage): `preg_replace('/(?<!\\\\)u0022/', '\\u0022', $content)` — the negative lookbehind avoids double-prefixing already-correct tokens.

**VB round-trip compatibility (Divi 5.10.1, verified 2026-08-13).** A no-semantic-change Visual Builder save preserved full color-variable payloads on the tested `divi/heading` background, border, and spacing paths without introducing doubled quote escapes. Reopening VB and loading the frontend retained the resolved background/border styling with no literal token or `0022` leak. This closes only those exact Heading paths; other modules and path variants remain unverified because the historical corruption was path-specific. Keep the MCP token normalizer unchanged as an input-safety guard and recovery path for malformed caller input or older content.
...
MCP `diviskit_preset_create` writes all three buckets uniformly (full-mirror — see "Full-mirror mitigation" below). This is intentional for Pass-B-consumer consistency, but the bucket-shape divergence from VB is a real difference. Diff/audit tools comparing VB-authored vs MCP-authored presets must not treat missing `renderAttrs` as drift — it's a structural choice on VB's side.
...
**VB preset-apply semantics (Divi 5.10.1, corrected matrix verified 2026-08-13).** The exact `divi/heading` spacing results are `module.decoration.spacing` `{first-apply: strip, replacement: keep}` and `title.decoration.spacing` `{first-apply: keep, replacement: keep}`. Every replacement used a fresh distinguishable inline margin while preset A remained bound; both inner-title actions used their own fresh values. The kept values persisted through full VB reloads and matched authenticated frontend styles. This proves that inline stripping varies by exact slot and slot history; do not infer behavior for other modules or dotted paths. Retain explicit matching-inline removal in MCP writes.
...
```
This stacks oa Heading H2 (size/weight/lineHeight) + oa Heading Light (color) — the color from the second preset overlays the first. Resolve `<role-key>` placeholders from `.devin/skills/diviskit-builder/design-system.json`.
...
**`dynamicOptionGroups` is in the `preset_reassign` reserved-keys list** — strip-inline never removes it (`diviskit-agent.php:strip_reserved_keys()`).
...
> **Canonical target model**: The preset names, role keys, and token references below define the **target state** every oa design system project should bootstrap toward. The preset UUIDs are site-specific — resolve via `.devin/skills/diviskit-builder/design-system.json` or `diviskit_preset_audit`. Projects that haven't bootstrapped yet won't have these presets; the agent falls back to inline styling.
...
> Replace `<role-key>` placeholders with the actual UUID from `.devin/skills/diviskit-builder/design-system.json` (e.g. `<heading-h1>` → look up `presets.heading-h1.id`).
...
- `GET /diviskit/v1/preset/list` — Read all presets (D5 + legacy)
- `GET /diviskit/v1/preset/audit` — Audit with referenced/unreferenced analysis
- `GET /diviskit/v1/preset/scan-orphans` — List UUIDs referenced in pages but missing from the D5 registry; separates dangling orphans from D4-legacy candidates
- `POST /diviskit/v1/preset/cleanup` — Remove orphans, rename, dedup (dry_run default)
- `POST /diviskit/v1/preset/create` — Create a new preset (module or group). Supported module types include `divi/button`, `divi/heading`, `divi/text`, `divi/blurb`, `divi/section`, `divi/row`, `divi/column`, `divi/group`, and any other type Divi tracks in the D5 registry. The `attrs` shape differs by `type`: `module` uses the **full module top-level attrs tree** (e.g. `{module: {decoration: {...}}, content: {...}}`); `group` uses the **fragment for that attribute group only** (e.g. `{title: {decoration: {font: {...}}}}` for a font preset on a heading's designTitleText slot). Response payload returns the created UUID as `preset.id` (nested under a `preset` object)
- `POST /diviskit/v1/preset/update` — Update single preset (name, attrs)
...
> - **Practical implication today**: MCP can author `divi/spacing` group presets and the user can apply them in VB without runtime issues. The corrected Divi 5.10.1 Heading matrix stripped inline only for module-level first apply; module-level replacement and both tested inner-title histories kept their freshly seeded inline values. Headless writes must still clear the matching inline attr in the same call because any survivor wins the cascade, and other modules, dotted slots, and older versions remain outside this proof.

> **`module_update` literal-dot key syntax for Composable Settings slots** *(verified 2026-05-04)*: by default `diviskit_module_update` parses `attrs` keys as dot-notation paths. Composable Settings preset slots use **literal-dot keys** like `groupPreset["title.decoration.spacing"]` where the dots are part of the key, not path separators. Escape inner dots with `\.` to embed them literally:
>
> ```jsonc
...
- `POST /diviskit/v1/preset/delete` — Delete single preset
...
1. `diviskit_preset_scan_orphans` — identify UUIDs referenced in pages but missing from the registry (dangling deletions or D4-legacy refs)
2. `diviskit_preset_create` — create a fresh named preset with the desired shared attrs (e.g. `module_name: "divi/column"`, `name: "White Card Surface"`, `attrs: { module: { decoration: { background: ..., spacing: ..., border: ... } } }`) — the created UUID is returned as `preset.id` in the response; use that for the next step's `new_uuid` parameter
3. `diviskit_preset_reassign` with `mode: "dry-run"` — preview which pages/modules would swap the orphan UUID → `new_uuid` and which inline attrs would be stripped
4. `diviskit_preset_reassign` with `mode: "apply"` — actually rewrite the pages; content goes through `parse_blocks` + `serialize_blocks` (no regex surgery) so only `modulePreset` arrays and redundant inline attrs are touched

**Purely-inline modules (no existing UUID)** have no automated batch migration path today. `diviskit_preset_reassign` keys off `old_uuid`, so modules with no `modulePreset` entry can't be batch-attached to a freshly-created preset. The manual workflow:

1. `diviskit_page_get` (or `diviskit_page_get_layout`) — find the inline modules to consolidate; pick one as the seed
2. `diviskit_preset_create` — create the preset using the seed module's inline attrs → the new preset UUID comes back as `preset.id` in the response
3. For each previously-inline module, call `diviskit_module_update` with `attrs: { modulePreset: ["<preset.id>"] }` to attach the preset reference. Inline attrs that duplicate the preset's values can be cleared in the same call by setting them to `null`, or left in place (inline wins over preset, so duplicates are harmless but redundant)
...
**Attribute-level (`groupPreset`) consolidation** across pages is now supported directly by `diviskit_preset_reassign` — pass a group-bucket `new_uuid` (with scope=`"both"` default) and the tool walks `attrs.groupPreset.<slot>.presetId` in page content plus `attrs.groupPresets.<slot>.presetId` registry chain refs in one call. Inline-strip for group scope is not yet implemented; UUID swaps happen, inline attrs are untouched (the summary includes a `strip_advisory` note when this applies). Manual follow-up with `diviskit_module_update` is still required if you want to clear redundant inline attrs on the matching slot's decoration subtree.

### When to Use Presets vs Inline Styles

**If you're starting a new project, presets are optional.** Diviskit generates polished pages using hardcoded values from [design-guide.md](design-guide.md) patterns without any preset setup. See [SKILL.md "First time on a project?"](../SKILL.md#first-time-on-a-project-start-here) for the shortest path.
...
Each project stores a `.devin/skills/diviskit-builder/design-system.json` file (NOT in the skill directory, NOT shipped to dist) that maps role keys to site-specific preset UUIDs. Generated by the bootstrap workflow in SKILL.md.
