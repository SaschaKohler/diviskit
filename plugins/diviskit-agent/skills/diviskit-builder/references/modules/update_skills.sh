...
# The config may name the server "diviskit-mcp" (or any other key — any
# server entry carrying a WP_URL env is used as fallback).
...
# Emits shell-quoted assignments; tries diviskit-mcp, then any server
# entry carrying a WP_URL env.
CREDS="$(python3 - "$MCP_CONFIG" <<'PY'
