---
name: diviskit
description: Diviskit harness primer — error envelope, capability handshake, dry-run plan shape, idempotency contract. Used as a shared primer by Diviskit target-coverage skills.
metadata:
  author: diviskit-dev
  version: "1.0"
---

# Diviskit harness primer

Cross-cutting conventions every Diviskit target-coverage skill (`diviskit-builder`, `diviskit-scf`, `diviskit-vendokit`, future slices) relies on. Read this when you're about to issue a `diviskit_*` MCP call and want to know what the success / failure envelope looks like, how to preview a write, or which capability gate decides whether a tool is even registered for this session.

This skill carries no tool-surface documentation — tools live with the coverage slice that owns them.

## Multi-LLM usage

These skills are **client- and model-agnostic**. The integration point is the Diviskit MCP server (registered once in your Devin `mcp_config.json`), not any specific LLM. Any model driving Devin can issue `diviskit_*` calls; the envelope, error codes, and `dry_run` contract below are identical regardless of which model is active.

Model choice affects **judgement, not the contract**:
- **Page authoring / design work** (`diviskit-builder`) benefits from a more capable model — block-format correctness, animation/hover/responsive coverage, and VB-round-trip safety reward stronger reasoning.
- **Lookups & contract reference** (this primer, `diviskit-scf` reads, `diviskit-vendokit` reads) are lightweight; a faster model is fine.
- **Mutating writes** should always go through `dry_run: true` first regardless of model — the plan preview is the safety gate, not the model's confidence.

Do NOT add a `model:` override to these skills if you want to switch LLMs freely per session — a hardcoded model frontmatter field would pin a single model and defeat that flexibility. Leave the model to the session/CLI flag.

## Response envelope

Every `diviskit_*` tool returns the same envelope:
...
`capability_missing` is the handshake-layer signal — the plugin on this site doesn't carry the capability flag the tool requires. MCP server and WordPress plugin versions are independent; install a compatible plugin component from the same Diviskit suite release or a newer supported component, then reconnect or restart the MCP session to refresh the handshake. Distinct from `forbidden` (the WP user lacks the capability on this row). Do not infer an exact required plugin version unless authoritative release-manifest evidence is actually supplied.
...
- `diviskit_meta_wp_cli` — raw passthrough; use explicit read-only commands instead.
- `diviskit_scf_import` — SCF's upstream `wp scf json import` lacks a `--dry-run` flag; use `diviskit_scf_sync --dry_run` for SCF-on-disk previews.

`diviskit_scf_sync` flows `dry_run` through to wp-cli's `--dry-run` flag — the resulting preview is wp-cli's plain-text summary, NOT the standardized `data.plan = { summary, changes[] }` shape above. This divergence is by design because it reports the upstream SCF sync preview.
...
A tool is idempotent when running it twice produces the same observable state as running it once. Diviskit tools document idempotency per-tool; the conventions:
...
- A `diviskit_*` tool returned `{ ok: false, error: { code: "scf.not_configured", … } }` and you want to know whether to surface a setup hint or a per-call recovery hint → gate code, setup-side fix.
...
When you need the actual tool list for a specific target, route to the coverage-slice skill (`diviskit-builder` for Divi page authoring, `diviskit-scf` for SCF data ops, future slices for future targets). The primer is the contract; the slice is the surface.
