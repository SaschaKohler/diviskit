...
> **Don't use this plugin standalone** — it's the WordPress side of a two-piece suite; install + configure the [Diviskit MCP Server](../../../diviskit-server/) next.
...
1. Zip this directory: `cd wp-content/plugins && zip -r diviskit-agent.zip diviskit-agent/`
2. **WP Admin → Plugins → Add New → Upload Plugin** — upload `diviskit-agent.zip` and activate.
...
1. Download `diviskit-agent.zip` from the public dist repo root.
2. Go to **WP Admin → Plugins → Add New → Upload Plugin**.
3. Upload the new `diviskit-agent.zip`.
4. Choose **Replace current with uploaded** when WordPress asks.

Your Application Password and MCP client config stay unchanged across Free plugin updates. Purchased Pro users activate Pro update access separately in **Diviskit → Pro License**.
...
alignment in the Diviskit dashboard on WordPress 7.1. This is a presentation-only
...
```bash
git diff --check
php -l wp-content/plugins/diviskit-agent/diviskit-agent.php
```

Then run the official WordPress.org readme validator against `wp-content/plugins/diviskit-agent/readme.txt` and run Plugin Check on the packaged plugin. For WordPress.org submission, use `diviskit-agent` as the directory slug/text domain target. If WP-CLI is available in the target environment, the Plugin Check command shape is:

```bash
wp plugin install plugin-check --activate
wp plugin check diviskit-agent --categories=plugin_repo
```
...
Communication is via the `/diviskit/v1/*` REST namespace, authenticated with Application Passwords. The MCP server reads the plugin's per-tool capability map at startup (the `/handshake` endpoint) and only exposes tools the plugin advertises support for — so you can update the plugin and server independently and unsupported tools fail with a clear `capability_missing` error rather than silent runtime breakage.
...
See the [Diviskit MCP Server README](../../../diviskit-server/) for full setup and the response contract.
...
## Schema dump & skill references

`GET /wp-json/diviskit/v1/schema/module/dump-all` returns every registered `divi/*` block's full attribute schema plus a `schema_version` fingerprint — a SHA-1 over Divi's `*PresetAttrsMap.php` files, so it changes exactly when canonical attribute paths change (i.e. after a Divi theme update). Related read endpoints: `/schema/modules` (list) and `/schema/module/{name}` (single module).

To regenerate the local `diviskit-builder` skill references, run one command from anywhere inside the DDEV project:

```bash
~/.config/devin/skills/diviskit-builder/references/modules/update_skills.sh
```

The script searches upward for `.devin/mcp_config.local.json` (server key `diviskit-mcp` or `diviops-mcp`), fetches the dump via REST, and regenerates `references/modules/*`, `module-formats.md` and the `divi-version` in `SKILL.md`. The WordPress dashboard card (**Diviskit Agent → Schema dump**) shows the live fingerprint, the copyable command, and a manual JSON download as fallback.
...
2. Install or copy `diviskit-agent/`.
3. Activate `Diviskit Agent`.
4. Keep your MCP server config pointed at `/wp-json/diviskit/v1/`; the REST namespace is unchanged.

## Learn more

- [Diviskit MCP Server README](../../../diviskit-server/) — server quick start + response contract
