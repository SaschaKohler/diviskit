<?php
/**
 * Trait Diviskit_Agent_Variable
 *
 * Variable CRUD, fluid system generator, orphan scan, page usage.
 *
 * Mixed into Diviskit_Agent via `use` in diviskit-agent.php — `self::` calls and
 ...
 *   - Diviskit_Variable_Input_Exception → `invalid_input` envelope
 ...
 */
if ( ! class_exists( 'Diviskit_Variable_Input_Exception' ) ) {
	class Diviskit_Variable_Input_Exception extends \InvalidArgumentException {}
}
...
trait Diviskit_Agent_Variable {
	...
	private static function generate_fluid_clamp_from_minmax( $min_str, $max_str, $output_unit = 'px', $root_font_size_px = 16.0 ) {
		...
		if ( null === $min_p ) {
			...
			throw new \Diviskit_Variable_Input_Exception( sprintf(
	...
	private static function generate_fluid_clamp_from_targets( $targets, $output_unit = 'px', $root_font_size_px = 16.0 ) {
		if ( ! is_array( $targets ) || 2 !== count( $targets ) ) {
			throw new \Diviskit_Variable_Input_Exception( "targets must contain exactly 2 entries keyed by viewport width (e.g. {'320px':'20px','1920px':'60px'})." );
		}
		$points = [];
	...
	public static function variable_create( $request ) {
		...
		if ( $has_fluid ) {
			...
			} catch ( \Diviskit_Variable_Input_Exception $e ) {
	 ...
	 * - "wide": 320/1920 — diviskit convention covering wider device span; matches the
		...
		switch ( $profile ) {
			...
			case 'custom':
				if ( ! is_array( $custom_anchors ) ) {
					throw new \Diviskit_Variable_Input_Exception( 'profile="custom" requires custom_anchors: {min_viewport_px, max_viewport_px}.' );
				}
				...
				if ( $min_vp <= 0 || $max_vp <= 0 || $max_vp <= $min_vp ) {
					throw new \Diviskit_Variable_Input_Exception( 'custom_anchors must provide positive min_viewport_px and max_viewport_px with max > min.' );
				}
	...
	private static function compute_typography_scale( $cfg, $anchors, $output_unit, $root_font_size_px, $namespace ) {
		...
		if ( $base_px <= 0 ) {
			throw new \Diviskit_Variable_Input_Exception( 'typography.base_px must be a positive number (px).' );
		}
		if ( $steps < 1 || $steps > 20 ) {
			throw new \Diviskit_Variable_Input_Exception( 'typography.steps must be between 1 and 20.' );
		}
		if ( $fluid_growth <= 0 ) {
			throw new \Diviskit_Variable_Input_Exception( 'typography.fluid_growth must be a positive number (e.g. 1.0 for non-fluid, 1.25 for moderate growth).' );
		}
	...
	private static function resolve_modular_ratio( $ratio_input ) {
		if ( is_numeric( $ratio_input ) ) {
			$r = (float) $ratio_input;
			if ( $r <= 0 ) {
				throw new \Diviskit_Variable_Input_Exception( 'Modular ratio must be a positive number.' );
			}
			return $r;
		}
		...
		throw new \Diviskit_Variable_Input_Exception( 'Modular ratio must be a number or a named scale.' );
	}
	 ...
	 * wide (320/1920) matches the diviskit convention, custom takes explicit
