 ...
 * the semantic field names exposed by Diviskit_Agent_SEO.
 */
final class Diviskit_SEO_TSF_Adapter {
	const ID                    = 'tsf';
...
trait Diviskit_Agent_SEO {
	public static function seo_provider_list( $request ) {
		return self::envelope_success( [
			'providers' => [ Diviskit_SEO_TSF_Adapter::discovery() ],
			'count'     => 1,
			'selection' => [
				'default' => 'auto',
	...
	public static function seo_metadata_get( $request ) {
		...
		try {
			$state     = Diviskit_SEO_TSF_Adapter::read_fields( $post_id );
			$effective = Diviskit_SEO_TSF_Adapter::effective_fields( $post_id );
		} catch ( Throwable $error ) {
	...
	public static function seo_metadata_update( $request ) {
		...
		try {
			$before                  = Diviskit_SEO_TSF_Adapter::read_fields( $post_id );
			$before_registered_state = Diviskit_SEO_TSF_Adapter::read_registered_state( $post_id );
		} catch ( Throwable $error ) {
			return self::seo_provider_runtime_error( $error, $post_id );
		}
		...
		if ( ! hash_equals( $before_checksum, $expected_checksum ) ) {
			return self::envelope_error(
				...
				[
					'post_id'           => $post_id,
					'provider'          => Diviskit_SEO_TSF_Adapter::ID,
		...
		$target_registered_state = Diviskit_SEO_TSF_Adapter::with_v1_state( $before_registered_state, $target );
		...
		if ( $dry_run ) {
			...
			return self::dry_run_response(
				...
				[],
				[
					'provider'          => [ 'id' => Diviskit_SEO_TSF_Adapter::ID, 'version' => $provider['version'] ],
					'current_checksum'  => $before_checksum,
		...
		try {
			foreach ( $touched as $field ) {
				Diviskit_SEO_TSF_Adapter::write_field(
					$post_id,
					$field,
			...
			$readback                  = Diviskit_SEO_TSF_Adapter::read_fields( $post_id );
			$registered_state_readback = Diviskit_SEO_TSF_Adapter::read_registered_state( $post_id );
		} catch ( Throwable $error ) {
		...
		if ( ! self::seo_states_equal( $readback, $target )
			|| ! Diviskit_SEO_TSF_Adapter::registered_states_equal( $registered_state_readback, $target_registered_state ) ) {
			return self::seo_apply_failure_with_rollback(
				$post_id,
				$before,
				$before_registered_state,
	...
	private static function seo_resolve_provider( $requested ) {
		if ( ! is_string( $requested ) || ! in_array( $requested, [ 'auto', Diviskit_SEO_TSF_Adapter::ID ], true ) ) {
			return self::envelope_error(
				'seo.provider_unsupported',
				'The requested SEO provider has no accepted Diviskit adapter.',
		...
		$provider = Diviskit_SEO_TSF_Adapter::discovery();
		if ( ! $provider['installed'] || ! $provider['active'] ) {
			return self::envelope_error(
				...
				'Install and activate a supported TSF version through normal WordPress administration, then retry. Diviskit never loads an inactive provider.',
				412,
				[
					'provider'  => Diviskit_SEO_TSF_Adapter::ID,
		...
		if ( ! $provider['compatible'] || ! $provider['readable'] || ! $provider['writable'] ) {
			return self::envelope_error(
				...
				'The active The SEO Framework version does not expose the validated Diviskit adapter contract.',
	...
	private static function seo_require_supported_post_type( $post ) {
		try {
			$supported = Diviskit_SEO_TSF_Adapter::supports_post_type( (string) $post->post_type );
		} catch ( Throwable $error ) {
	...
	private static function seo_prepare_changes( array $changes, array $before ): array {
		...
		foreach ( $changes as $change ) {
			...
			try {
				$sanitized = Diviskit_SEO_TSF_Adapter::sanitize( $validation['value'] );
			} catch ( Throwable $error ) {
				return [ 'error' => [
					'code'    => 'wp_error',
	...
	private static function seo_validate_plain_text( $value, string $field ): array {
		...
		if ( self::seo_looks_like_markup( $value ) ) {
			return [ 'error' => [
				...
				'hint'    => 'Pass plain text only; Diviskit refuses markup before provider sanitization.',
	...
	private static function seo_checksum( int $post_id, string $provider_version, array $state ): string {
		...
		$payload = [
			'post_id'  => $post_id,
			'provider' => [ 'id' => Diviskit_SEO_TSF_Adapter::ID, 'version' => $provider_version ],
			'fields'   => $fields,
		];
	...
	private static function seo_read_payload( $post, array $provider, array $state, array $effective ): array {
		return [
			...
			'provider'  => [
				'id'           => Diviskit_SEO_TSF_Adapter::ID,
				'name'         => Diviskit_SEO_TSF_Adapter::NAME,
				'version'      => $provider['version'],
	...
	private static function seo_update_success_payload( $post, array $provider, string $before_checksum, string $after_checksum, array $changes, array $state, bool $noop, bool $write_applied ): array {
		return [
			'post'                                 => self::seo_post_evidence( $post ),
			'provider'                             => [ 'id' => Diviskit_SEO_TSF_Adapter::ID, 'version' => $provider['version'], 'adapter' => $provider['adapter'] ],
	...
	private static function seo_apply_failure_with_rollback( int $post_id, array $before, array $before_registered_state, array $target_registered_state, ?Throwable $cause, ?array $mismatched_readback, ?array $registered_state_readback ) {
		...
		if ( null !== $registered_state_readback ) {
			$data['provider_state_changed_field_count'] = Diviskit_SEO_TSF_Adapter::registered_state_diff_count(
				$target_registered_state,
				$registered_state_readback
			);
		}
	...
	private static function seo_restore_before_state( int $post_id, array $before, array $before_registered_state ): array {
		...
		try {
			Diviskit_SEO_TSF_Adapter::restore_registered_state( $post_id, $before_registered_state );
			$restored                  = Diviskit_SEO_TSF_Adapter::read_fields( $post_id );
			$registered_state_restored = Diviskit_SEO_TSF_Adapter::read_registered_state( $post_id );
			$result['provider_registered_state_restored'] = Diviskit_SEO_TSF_Adapter::registered_states_equal(
	...
	private static function seo_provider_runtime_error( Throwable $error, int $post_id ) {
		return self::envelope_error(
			...
			500,
			[ 'post_id' => $post_id, 'provider' => Diviskit_SEO_TSF_Adapter::ID, 'detail' => self::seo_safe_error_message( $error ) ]
		);
	}
