...
(function () {
    'use strict';
    const root = document.getElementById('diviskit-design-system');
    if (!root || !window.diviskitDesignSystem) return;
    const config = window.diviskitDesignSystem;
    ...
    function swatch(value) {
        ...
        const node = el('span', undefined, 'diviskit-ds-swatch');
        node.style.backgroundColor = value;
        node.setAttribute('aria-hidden', 'true');
        return node;
    }
    function variableValue(target, item) {
        const line = el('div', undefined, 'diviskit-ds-value');
    ...
    function renderList() {
        ...
        rows.slice(0, limit).forEach(row => {
            const item = el('li');
            const control = button('', () => select(row), 'diviskit-ds-entry');
