...
Connect Codex or Claude to WordPress through MCP. Build and improve editable Divi 5 pages with the Diviskit AI harness.

== Description ==

Work on your WordPress website with Codex, Claude Code, Claude Desktop or another compatible MCP client. Diviskit Agent connects the Diviskit AI harness to your site, so your agent can inspect existing content and help build and improve native, editable Divi 5 pages.

Diviskit is in public beta. It targets Divi 5 today, with WordPress as the wider foundation. Your agent works in its own client; this plugin provides the authenticated WordPress REST bridge.
...
Keep your audience, page goals, design references and review steps in your AI client's project documentation. Use the Diviskit authoring skills alongside that context to guide repeatable workflows. The WordPress plugin does not store or manage those client-side documents.

= How the pieces fit together =

Your AI client runs the separately installed Diviskit MCP server. That server connects to this plugin using a WordPress Application Password. The plugin's WordPress dashboard shows installed components and supported status information; it is not an AI chat interface or proof of an active MCP connection.

Start with a staging site, ask the agent to inspect before editing, and review the result before publishing.

[Setup documentation](https://diviskit.com/docs/) | [Real workflows](https://diviskit.com/use-cases/)
...
Diviskit Agent is a WordPress REST bridge. Normal Free plugin runtime does not require the plugin to contact Diviskit servers.

To use the plugin, you run the separately distributed `@diviskit/mcp-server` package, which is published through npm. Depending on your installation method, `npx` or npm may download that package from the npm registry. The MCP server then connects to your WordPress site with WordPress Application Password authentication.

Relevant external service:

* Service: npm registry, used to distribute `@diviskit/mcp-server`
* Package: https://www.npmjs.com/package/@diviskit/mcp-server
...
The plugin does not add analytics or tracking. It exposes authenticated REST endpoints on your WordPress site. What data is read or written depends on the MCP tools you choose to run, your WordPress user's permissions, and the installed Diviskit plugin capabilities.

== Installation ==

1. Install **Diviskit Agent** from **Plugins > Add New**, or upload `diviskit-agent.zip`.
...
Yes. The Diviskit MCP server connects compatible clients including Codex, Claude Code and Claude Desktop to this plugin. Client configuration and client access are separate from the WordPress plugin.
...
Yes. Diviskit Agent targets Divi 5 today. Authenticated requests return a `divi_unavailable` error when Divi is not active.
...
= Is every Diviskit MCP tool included in this Free plugin? =
...
= How do I refresh the Divi module schema after a Divi update? =

Divi theme updates can change module attribute paths. The plugin exposes the full schema at `GET /wp-json/diviskit/v1/schema/module/dump-all`, including a `schema_version` fingerprint that changes exactly when Divi's canonical attribute maps change.

The Diviskit dashboard card (**Diviskit Agent > Schema dump**) shows the live fingerprint and the refresh command:

`~/.config/devin/skills/diviskit-builder/references/modules/update_skills.sh`

Run it from anywhere inside your DDEV project (or pass the project directory as the first argument). The script reads credentials from `.devin/mcp_config.local.json`, fetches the dump itself, and regenerates the local skill references. The card's JSON download button is an optional manual fallback.

== Screenshots ==

1. Diviskit Agent Free dashboard showing installed components, rate limits and setup information. Optional add-ons are not active in this capture.
...
* Adds typed WordPress menu tools for creating menus, adding page/custom-link items, reading normalized menu trees, and assigning registered theme locations through the Diviskit capability handshake.
...
* Adds richer Diviskit preflight metadata for the MCP server, including plugin version records used by `diviops_meta_info`.
* Keeps authenticated Diviskit REST endpoints and capability handshake support aligned with the current MCP server release.
