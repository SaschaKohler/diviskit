<?php
/**
 * Plugin Name: Diviskit Design Library
 * Description: Modern design effects for Divi 5 with Three.js, CSS animations, and reusable design elements.
 * Version: 1.0.0
 * Author: Diviskit
 * Requires at least: 6.0
 * Requires PHP: 7.4
 * Text Domain: diviskit-design-library
 * License: GPL-2.0-or-later
 * License URI: https://www.gnu.org/licenses/gpl-2.0.html
 * Update URI: https://diviskit.com/item/diviskit-design-library/
 *
 * Forked from the GPL-licensed diviops-design-library. Integration contract:
 * `dsk-*` CSS classes, `_diviskit_design_*` post-meta keys and
 * `diviskit-design-fx`/`threejs` script handles (renamed from `ddl-*` /
 * `_divi_design_*` / `divi-design-fx` — hard cutover, no legacy aliases).
 */

if ( ! defined( 'ABSPATH' ) ) {
	exit;
}

class Diviskit_Design_Library {

	const VERSION = '1.0.0';

	/**
	 * Three.js version to bundle.
	 */
	const THREEJS_VERSION = 'r128';

	/**
	 * Initialize the plugin.
	 */
	public static function init() {
		add_action( 'wp_enqueue_scripts', [ __CLASS__, 'register_scripts' ] );
		add_action( 'wp_enqueue_scripts', [ __CLASS__, 'maybe_enqueue' ] );
		add_action( 'wp_head',            [ __CLASS__, 'print_css_variables' ] );
	}

	/**
	 * Register scripts (but don't enqueue yet).
	 */
	public static function register_scripts() {
		$base_url = plugin_dir_url( __FILE__ ) . 'assets/js/';

		// Three.js — only loaded when needed.
		wp_register_script(
			'threejs',
			$base_url . 'three.min.js',
			[],
			self::THREEJS_VERSION,
			[ 'in_footer' => true ]
		);

		// Design library helpers — CSS animations, intersection observer, etc.
		wp_register_script(
			'diviskit-design-fx',
			$base_url . 'design-fx.js',
			[],
			self::VERSION,
			[ 'in_footer' => true ]
		);
	}

	/**
	 * Conditionally enqueue scripts based on page meta or global setting.
	 *
	 * Pages opt-in via:
	 * - Post meta: _diviskit_design_threejs = '1'
	 * - Post meta: _diviskit_design_fx = '1'
	 * - Or always load design-fx if the page uses Divi builder.
	 */
	public static function maybe_enqueue() {
		if ( ! is_singular() ) {
			return;
		}

		$post_id = get_queried_object_id();
		if ( ! $post_id ) {
			return;
		}

		// Always load design-fx on Divi pages (lightweight).
		if ( function_exists( 'et_pb_is_pagebuilder_used' ) && et_pb_is_pagebuilder_used( $post_id ) ) {
			wp_enqueue_script( 'diviskit-design-fx' );
		}

		// Three.js only when explicitly requested.
		if ( get_post_meta( $post_id, '_diviskit_design_threejs', true ) === '1' ) {
			wp_enqueue_script( 'threejs' );
		}

		// Also check if the content contains Three.js markers.
		// Divi stores HTML in block attributes with unicode escapes, so check broadly.
		$content = get_post_field( 'post_content', $post_id );
		if ( strpos( $content, 'webgl' ) !== false ||
		     strpos( $content, 'THREE' ) !== false ||
		     strpos( $content, 'shader' ) !== false ||
		     strpos( $content, 'three.js' ) !== false ) {
			wp_enqueue_script( 'threejs' );
		}
	}

	/**
	 * Print CSS custom properties and base animation styles.
	 */
	public static function print_css_variables() {
		if ( ! is_singular() ) {
			return;
		}

		$post_id = get_queried_object_id();
		if ( ! $post_id ) {
			return;
		}

		if ( ! function_exists( 'et_pb_is_pagebuilder_used' ) || ! et_pb_is_pagebuilder_used( $post_id ) ) {
			return;
		}

		?>
		<style id="diviskit-design-library">
			body.dsk-scroll-stage-active #page-container { overflow-y: visible !important; }

			/* ===== Entrance Animations ===== */
			@keyframes dsk-fade-up {
				from { opacity: 0; transform: translateY(30px); }
				to   { opacity: 1; transform: translateY(0); }
			}
			@keyframes dsk-fade-in {
				from { opacity: 0; }
				to   { opacity: 1; }
			}
			@keyframes dsk-scale-in {
				from { opacity: 0; transform: scale(0.9); }
				to   { opacity: 1; transform: scale(1); }
			}
			@keyframes dsk-slide-left {
				from { opacity: 0; transform: translateX(40px); }
				to   { opacity: 1; transform: translateX(0); }
			}
			@keyframes dsk-slide-right {
				from { opacity: 0; transform: translateX(-40px); }
				to   { opacity: 1; transform: translateX(0); }
			}

			/* Apply via CSS class — elements start hidden, animate when .dsk-visible is added */
			.dsk-animate {
				opacity: 0;
			}
			/* VB override — show all elements in the Visual Builder so they remain editable.
			   Only reset opacity and animation — do NOT reset transform (would break intentional transforms). */
			#et-fb-app .dsk-animate,
			.et-fb .dsk-animate,
			body.et-fb .dsk-animate {
				opacity: 1 !important;
				animation: none !important;
			}
			.dsk-animate.dsk-visible {
				animation-duration: 0.8s;
				animation-fill-mode: both;
				animation-timing-function: cubic-bezier(0.16, 1, 0.3, 1);
			}
			.dsk-fade-up.dsk-visible    { animation-name: dsk-fade-up; }
			.dsk-fade-in.dsk-visible     { animation-name: dsk-fade-in; }
			.dsk-scale-in.dsk-visible    { animation-name: dsk-scale-in; }
			.dsk-slide-left.dsk-visible  { animation-name: dsk-slide-left; }
			.dsk-slide-right.dsk-visible { animation-name: dsk-slide-right; }

			/* Stagger delays — add dsk-delay-1 through dsk-delay-6 */
			.dsk-delay-1.dsk-visible { animation-delay: 0.1s; }
			.dsk-delay-2.dsk-visible { animation-delay: 0.2s; }
			.dsk-delay-3.dsk-visible { animation-delay: 0.3s; }
			.dsk-delay-4.dsk-visible { animation-delay: 0.4s; }
			.dsk-delay-5.dsk-visible { animation-delay: 0.5s; }
			.dsk-delay-6.dsk-visible { animation-delay: 0.6s; }

			/* ===== Animated Gradient Background ===== */
			@keyframes dsk-gradient-shift {
				0%   { background-position: 0% 50%; }
				50%  { background-position: 100% 50%; }
				100% { background-position: 0% 50%; }
			}
			.dsk-gradient-animated {
				background-size: 200% 200% !important;
				animation: dsk-gradient-shift 8s ease infinite;
			}

			/* ===== Glass Morphism ===== */
			.dsk-glass {
				backdrop-filter: blur(12px) saturate(1.5);
				-webkit-backdrop-filter: blur(12px) saturate(1.5);
				background: rgba(255, 255, 255, 0.08) !important;
				border: 1px solid rgba(255, 255, 255, 0.12) !important;
			}
			.dsk-glass-light {
				backdrop-filter: blur(12px) saturate(1.5);
				-webkit-backdrop-filter: blur(12px) saturate(1.5);
				background: rgba(255, 255, 255, 0.7) !important;
				border: 1px solid rgba(255, 255, 255, 0.5) !important;
			}

			/* ===== Pulse indicator ===== */
			@keyframes dsk-pulse {
				0%   { transform: scale(1); opacity: 0.75; }
				100% { transform: scale(2.5); opacity: 0; }
			}
			.dsk-pulse-dot {
				position: relative;
				display: inline-flex;
				align-items: center;
				justify-content: center;
				width: 12px;
				height: 12px;
			}
			.dsk-pulse-dot::before {
				content: '';
				position: absolute;
				width: 100%;
				height: 100%;
				border-radius: 50%;
				background: currentColor;
				animation: dsk-pulse 1.5s ease-out infinite;
			}
			.dsk-pulse-dot::after {
				content: '';
				position: relative;
				width: 8px;
				height: 8px;
				border-radius: 50%;
				background: currentColor;
			}

			/* ===== Marquee (reusable) ===== */
			@keyframes dsk-marquee {
				0%   { transform: translateX(0); }
				100% { transform: translateX(-50%); }
			}
			.dsk-marquee-track {
				overflow: hidden;
			}
			.dsk-marquee-scroll {
				display: inline-flex !important;
				flex-wrap: nowrap !important;
				gap: 0 !important;
				animation: dsk-marquee var(--dsk-marquee-speed, 30s) linear infinite;
			}
			.dsk-marquee-scroll > * {
				flex-shrink: 0;
				white-space: nowrap;
			}
			.dsk-marquee-scroll:hover,
			.dsk-marquee-scroll:focus-within {
				animation-play-state: paused;
			}
			@media (prefers-reduced-motion: reduce) {
				.dsk-marquee-track { overflow-x: auto; }
				.dsk-marquee-scroll { animation: none; }
			}

			/* ===== Gradient Text ===== */
			.dsk-gradient-text {
				background: linear-gradient(135deg, #6366f1 0%, #ec4899 50%, #f59e0b 100%);
				-webkit-background-clip: text;
				-webkit-text-fill-color: transparent;
				background-clip: text;
			}
			.dsk-gradient-text-animated {
				background: linear-gradient(135deg, #6366f1, #ec4899, #f59e0b, #10b981, #6366f1);
				background-size: 300% 300%;
				-webkit-background-clip: text;
				-webkit-text-fill-color: transparent;
				background-clip: text;
				animation: dsk-gradient-shift 6s ease infinite;
			}

			/* ===== Text Stroke / Outline ===== */
			.dsk-text-stroke {
				color: transparent !important;
				-webkit-text-stroke: 2px rgba(255,255,255,0.25);
			}
			.dsk-text-stroke-dark {
				color: transparent !important;
				-webkit-text-stroke: 2px rgba(0,0,0,0.15);
			}

			/* ===== Gooey Text Morph (SVG filter) ===== */
			.dsk-gooey-wrap {
				position: relative;
				display: flex;
				align-items: center;
				justify-content: center;
				filter: url(#dsk-gooey-filter);
			}
			.dsk-gooey-text {
				position: absolute;
				display: inline-block;
				font-weight: 900;
				text-align: center;
				user-select: none;
			}

			/* ===== Hover lift effect ===== */
			.dsk-hover-lift {
				transition: transform 0.3s cubic-bezier(0.16, 1, 0.3, 1), box-shadow 0.3s ease;
			}
			.dsk-hover-lift:hover {
				transform: translateY(-4px);
				box-shadow: 0 12px 40px rgba(0, 0, 0, 0.12);
			}
		</style>
		<?php
	}
}

// Compat shim — the agent plugin's status card and any third-party code may
// still look for the pre-fork class name.
if ( ! class_exists( 'DiviOps_Design_Library', false ) ) {
	class_alias( 'Diviskit_Design_Library', 'DiviOps_Design_Library' );
}

/**
 * License + update client. Talks to the store's ?vendokit-license=* API.
 * Store URL: define DSK_DESIGN_LIBRARY_STORE_URL in wp-config.php to
 * override, or filter dsk_design_library_store_url.
 */
if ( file_exists( __DIR__ . '/includes/class-diviskit-license-client.php' ) ) {
	require_once __DIR__ . '/includes/class-diviskit-license-client.php';

	Diviskit_License_Client::register( array(
		'item'         => (string) apply_filters( 'dsk_design_library_license_item', 'diviskit-design-library' ),
		'api_url'      => defined( 'DSK_DESIGN_LIBRARY_STORE_URL' )
			? DSK_DESIGN_LIBRARY_STORE_URL
			: apply_filters( 'dsk_design_library_store_url', 'https://diviskit.com' ),
		'version'      => Diviskit_Design_Library::VERSION,
		'file'         => __FILE__,
		'slug'         => 'diviskit-design-library',
		'plugin_title' => 'Diviskit Design Library',
		'purchase_url' => apply_filters( 'dsk_design_library_purchase_url', 'https://diviskit.com/item/diviskit-design-library/' ),
		'free'         => true,
	) );
}

Diviskit_Design_Library::init();
