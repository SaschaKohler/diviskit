 ...
 * already opt-in via DIVISKIT_WP_CLI_ALLOW; their validation is tracked
 ...
 *
 * Escape hatch: `DIVISKIT_WP_CLI_UNSAFE_FS=1` disables all validation for
...
// Canonical names are DIVISKIT_*; the DIVIOPS_* variants stay accepted as
// undocumented aliases for configs written before the rename.
const UNSAFE_FS_ENV = 'DIVISKIT_WP_CLI_UNSAFE_FS';
const SAFE_FS_ROOT_OVERRIDE_ENV = 'DIVISKIT_WP_CLI_SAFE_FS_ROOT';
const UNSAFE_FS_ENVS = [UNSAFE_FS_ENV, 'DIVIOPS_WP_CLI_UNSAFE_FS'];
const SAFE_FS_ROOT_OVERRIDE_ENVS = [
  SAFE_FS_ROOT_OVERRIDE_ENV,
  'DIVIOPS_WP_CLI_SAFE_FS_ROOT',
];
...
export interface FsValidationOptions {
   ...
   * validator requires an explicit DIVISKIT_WP_CLI_SAFE_FS_ROOT before
 ...
 * Precedence: DIVISKIT_WP_CLI_SAFE_FS_ROOT env var > <wpPath>/.diviops-tmp/.
 * Returned path is canonical absolute.
 */
export function resolveSafeFsRoot(wpPath: string): string {
  for (const env of SAFE_FS_ROOT_OVERRIDE_ENVS) {
...
export function fsValidationDisabled(): boolean {
  for (const env of UNSAFE_FS_ENVS) {
...
export function validateFilesystemFlags(
  ...
  // caller to set DIVISKIT_WP_CLI_SAFE_FS_ROOT explicitly to the correct
  // container-namespace path before we'll validate FS-sensitive commands.
  const hasExplicitSafeRoot = SAFE_FS_ROOT_OVERRIDE_ENVS.some(
