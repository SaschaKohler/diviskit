...
export const META_INFO_CONFIG = {
  description:
    "Returns Diviskit MCP server identity, server_version, license type, numeric tool_count, registered tool catalog summary, active plugin version summary, WP-CLI allowlist, and plugin handshake/slice state including Pro and Vendokit target readiness. Use as the S0 preflight before dogfooding or product work. Returns the standardized envelope { ok, data?, error: { code, message, hint? } }.",
  annotations: { readOnlyHint: true, idempotentHint: true },
  _meta: { idempotent: "true" },
} as const;
...
export function registerLauncherHealthTools(options: {
  ...
  recordIdempotent("diviskit_meta_ping", META_PING_CONFIG._meta);
  recordIdempotent("diviskit_meta_info", META_INFO_CONFIG._meta);
  server.registerTool("diviskit_meta_ping", META_PING_CONFIG, async (
    _args: unknown,
    ...
    return modelVisibleHealthResult(response, target, "diviskit_meta_ping");
  });

  server.registerTool("diviskit_meta_info", META_INFO_CONFIG, async (
    _args: unknown,
    context: ToolCancellationContext | undefined,
  ) => {
    ...
    const response = await wrapResponse(async () => ({
      ...
      plugins: {
        diviskit_agent: { active: true, version: target.plugin_version },
      },
      handshake: {
        state: "ok",
    ...
    return modelVisibleHealthResult(response, target, "diviskit_meta_info");
  });
}
