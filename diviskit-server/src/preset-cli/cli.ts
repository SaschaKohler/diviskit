/**
 * `diviskit-preset` — standalone preset-emitter CLI.
 *
 ...
 *
 * Usage:
 *   diviskit-preset button [options]      Emit a divi/button group preset
 *   diviskit-preset nav [options]         Emit Divi nav block skeletons
 *   diviskit-preset --help                Show help
 *
 * Modes:
...
const HELP = `diviskit-preset — Divi 5.5.x canonical preset-emitter CLI

USAGE
  diviskit-preset button [options]           Emit a divi/button group preset
  diviskit-preset heading-font [options]     Emit a divi/font group preset for
                                            divi/heading
  diviskit-preset text-body-font [options]   Emit a divi/font-body group preset
                                            for divi/text (Pattern A only)
  diviskit-preset spacing [options]          Emit a divi/spacing group preset
...
EXAMPLES
  ...
  diviskit-preset button --name "Primary" --bg-color "#2563eb" --apply

  diviskit-preset heading-font --name "Heading H1" --pattern google \\
    --font-family Inter --font-weight 700 \\
    --font-color gcid-heading-color --font-size 48px

  diviskit-preset heading-font --name "Heading H1 (local)" --pattern local \\
    --font-family "Sora 700" \\
    --font-color gcid-heading-color --font-size 48px

  diviskit-preset text-body-font --name "Body Text" --pattern google \\
    --font-family Inter --font-weight 400 \\
    --font-color gcid-body-color --font-size 16px

  diviskit-preset spacing --name "Section Rhythm" --module divi/section \\
    ...
    if (!serverVersion) {
      io.err(
        "Apply mode requires the server version for the plugin handshake. " +
          "This is an internal error — invoke via the `diviskit-preset` bin.",
      );
