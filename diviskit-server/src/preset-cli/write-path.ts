...
/** The REST route the CLI posts to — same route `diviskit_preset_create` uses. */
...
export class CapabilityMissingError extends Error {
  constructor(
    ...
    public readonly tool: string = "diviskit-preset --apply",
  ) {
...
export class PresetIsolationError extends Error {
  constructor(public readonly refs: ForeignVarRef[]) {
    super(formatIsolationError("diviskit-preset --apply", refs));
