...
/**
 * Extended commands that require explicit opt-in via DIVISKIT_WP_CLI_ALLOW env var.
 ...
 *   DIVISKIT_WP_CLI_ALLOW="option update,post delete,search-replace"
 *
...
function buildAllowlist(): readonly string[] {
  const extra = (
    process.env.DIVISKIT_WP_CLI_ALLOW ?? process.env.DIVIOPS_WP_CLI_ALLOW
  )?.trim();
  ...
  if (extra === '*' || extra === 'all') {
    console.warn(
      `[diviskit] DIVISKIT_WP_CLI_ALLOW="${extra}" — granting ALL ${EXTENDED_COMMANDS.length} extended commands. Intended for trusted local-dev only.`,
    );
  ...
  for (const cmd of requested) {
    if (EXTENDED_COMMANDS.includes(cmd)) {
      ...
      console.warn(`[diviskit] Ignoring unknown WP-CLI allow entry: "${cmd}"`);
    }
  }
...
function isCommandAllowed(args: string[]): { allowed: boolean; reason?: string } {
  ...
  const hint = extendable.some((c) => twoWord === c || threeWord === c || oneWord === c.split(' ')[0])
    ? ` This command can be enabled via DIVISKIT_WP_CLI_ALLOW env var (see README).`
    : extendable.length > 0
...
export function createWpCli(config: WpCliConfig) {
  ...
  const runArgv = async (args: string[]): Promise<RunArgvResult> => {
    ...
    // (`import`, `eval-file`) are opt-in via DIVISKIT_WP_CLI_ALLOW, so opting
    // in signals the caller accepts path-scope risk. Skip entirely when the
    // user explicitly disables via DIVISKIT_WP_CLI_UNSAFE_FS=1.
    //
    ...
    // explicit DIVISKIT_WP_CLI_SAFE_FS_ROOT there. We also skip the host-side
