...
/**
 * Shape returned by `POST /diviskit/v1/handshake`.
 *
 * `capabilities` is a per-tool map keyed by post-rename tool slug
 * (without the `diviskit_` prefix). Older plugins (pre-1.2.0) emit
...
export interface HandshakeResult {
  ...
  pro_version?: string;
  /** ADR-003 — per-target presence map (Vendokit, future slices). */
...
export function proToolGatesSatisfied(
  ...
  for (const extra of gates.requiredCapabilities ?? []) {
    if (state.capabilities[extra] !== true) return false;
  }
  return true;
}
